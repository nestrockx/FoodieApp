...
import com.wegielek.mvvm_foodie.databinding.FragmentRandomRecipeBinding
...
class RandomRecipeFragment : Fragment(R.layout.fragment_random_recipe) {

    companion object {
        private const val TAG = "RandomRecipeFragment"
    }
           ...
           when (response) {
               is Resource.Success -> {
                   response.data?.let { recipeResponse ->
                       if (!recipeResponse.meals.isNullOrEmpty()) {
